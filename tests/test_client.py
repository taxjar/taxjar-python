...
class TestTaxJarClient(unittest.TestCase):
    def setUp(self):
        self.api_key = 'heythere'
        self.api_url = TaxJarClient.API_URL
        self.headers = { "Authorization": "Bearer heythere", "User-Agent": "TaxJarPython/0.0.1" }
        self.responder_mock = MagicMock()
        self.client = TaxJarClient(self.api_key, {}, self.responder_mock)
